  ...
  useEffect(() => {
    async function loadHostel() {
      try {
        ...
        const property = data.find((item) => String(item.id) === String(id));

        setHostel(property);

        setSelectedImage(`https://picsum.photos/900/650?random=${id}`);
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
...
rounded-xl shadow p-5"
          ...
          <div className="lg:w-[360px]">
            <div
              className="sticky top-24 bg-white rounded-2xl shadow-lg
p-8"
            >
